...
class EmailValidator{
     ...
     * @see allowName
     */
    private static $emailFullPattern = '/^[^@]*<[a-zA-Z0-9!#$%&\'*+\\/=?^_`{|}~-]+(?:\.[a-zA-Z0-9!#$%&\'*+\\/=?^_`{|}~-]+)*@(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?\.)+[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?>$/';
    /**
     * @var boolean whether to allow name in the email address (e.g. "John Smith <john.smith@example.com>"). Defaults to false.
     * @see fullPattern
     */
    private static $emailAllowName = false;
    /**
     ...
     */
    private static $emailCheckDNS = false;
    /**
     ...
     */
    private static $emailEnableIDN = false;
    ...
    public static function email($value){
        ...
        if (!is_string($value) || strlen($value) >= 320) {
            ...
            $domain = $matches[3];
            if (self::$emailEnableIDN) {
                $value = $matches[1] . idn_to_ascii($matches[2]) . '@' . idn_to_ascii($domain) . $matches[4];
            }
            $valid = preg_match(self::$emailPattern, $value) || self::$emailAllowName && preg_match(self::$emailFullPattern, $value);
            if ($valid && self::$emailCheckDNS) {
